...
def train_step(model: torch.nn.Module,
    ...
    for batch, (names, train_images, train_masks) in enumerate(data_loader):
        ...
        # 2.2 Class-wise loss
        class_wise_loss_batch = evaluation_utils.calculate_classwise_loss(train_logits, train_targets, num_classes, device)
        train_class_wise_loss_epoch += class_wise_loss_batch.to(device) # sum up for all batches in this epoch
...
def test_step(model: torch.nn.Module,
    ...
    with torch.inference_mode():
        ...
        # for batch_idx, (names, test_images, test_masks) in enumerate(data_loader):
            ...
            # 2.2 Class-wise loss
            class_wise_loss_batch = evaluation_utils.calculate_classwise_loss(test_logits, test_targets, num_classes, device)
            test_class_wise_loss_epoch += class_wise_loss_batch.to(device) # sum up for all batches in this epoch
...
#     # Turn on inference context manager
#     with torch.inference_mode():
#         for names, test_images, test_masks in tqdm(test_data_loader, desc="Making predictions"):
